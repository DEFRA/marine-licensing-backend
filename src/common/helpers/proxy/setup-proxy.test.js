...
if (parseInt(nodeVersion) < REQUIRED_NODE_VERSION) {
  describe('setup-proxy.js', () => {
    test.skip(`Tests skipped: requires Node.js v${REQUIRED_NODE_VERSION}+ (current: ${process.version})`, () => {
      console.warn(
        `⚠️ Tests for setup-proxy.js require Node.js v${REQUIRED_NODE_VERSION}+ but found ${process.version}`
      )
      console.warn('These tests will be skipped until you upgrade Node.js.')
    })
  })
} else {
  ...
  const setupProxyModule = require('./setup-proxy.js')
