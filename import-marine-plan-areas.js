...
function toCamelCase(str) {
  let cleaned = str.replace(/\s*inshore\s*/gi, '').trim()
  return cleaned
    .split(/\s+/)
    .map((word, i) =>
      i === 0
...
const geoJSONPath = join(__dirname, 'Marine_Plan_Areas.geojson')
...
geoJSON.features.forEach((feature) => {
  const baseName = toCamelCase(feature.properties.info)
...
const outputPath = join(__dirname, 'marine-plan-areas-import.json')
...
console.log(`3. Navigate to 'experiment-coastal-areas' collection`)
...
console.log(
  `   db.experiment-coastal-areas.createIndex({ geometry: "2dsphere" })`
)
